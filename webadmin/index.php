                        ...
                        <div class="content-container">
                            <div class="left-container" id="TABELITEM">
                                ...
                                <form id="checkoutForm">
                                    ...
                                    <button class="btn btn-success" style="background-color: #4e73df; margin-top: 10px;" type="submit">Checkout</button>
                                </form>
                            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#checkoutForm').on('submit', function(e) {
                e.preventDefault();
                let selectedRekening = $('#rekening').val();
                let totalValue = parseFloat($('#totalValue').text());

                if (selectedRekening && totalValue > 0) {
                    $.ajax({
                        url: 'checkout.php',
                        type: 'POST',
                        data: {
                    ...
                    alert('Please select a rekening and ensure total value is greater than 0.');
                }
            });
        });
        ...
        function loadRekeningOptions() {
            $.ajax({
                ...
                type: 'GET',
                success: function(data) {
                    let rekenings = JSON.parse(data);
                    rekenings.forEach(rekening => {
                        $('#rekening').append(new Option(rekening.namarekening, rekening.koderekening));
                    });
                }
            });
        }
